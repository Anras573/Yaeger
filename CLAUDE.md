...
- **`SceneLoader` / `Scene` / `SceneSaver`** — the scene pipeline. JSON files with a top-level `"entities"` array, each entry optionally carrying a `"tag"` plus its own `"components"` array. Loader reuses the same `ComponentRegistry` as prefabs. Instantiate via `world.Instantiate(scene)`, which returns `IReadOnlyList<Entity>`. Save a world back out with `new SceneSaver(registry).Save(world, path)`. `Scene.Apply` creates every entity (and registers its tag) before applying any component, so a component's deferred `Action<World, Entity>` can resolve another entity's tag regardless of file order — this is how `Parent` supports forward references. See `docs/scenes.md` and `Samples/SceneDemo`.
...
### Entity hierarchy (`ECS/Parent.cs`, `Graphics/LocalTransform2D.cs`/`LocalTransform3D.cs`, `Systems/TransformHierarchySystem.cs`)

`Transform2D`/`Transform3D` stay world-space everywhere else in the engine; only a hierarchy child carries the extra pieces. `Parent` (a single `Entity` reference — no separate child-list component; children are found by querying for `Parent`) paired with `LocalTransform2D`/`LocalTransform3D` (the child's transform relative to its parent) drives `TransformHierarchySystem : IUpdateSystem` (lives in `Yaeger.Core`, no `Window` dependency). Each `Update`, it orders parents before children (throwing `InvalidOperationException` on a cyclic `Parent` chain instead of hanging), composes each child's local transform with its parent's already-resolved world transform (scale-then-rotate-then-translate, matching `TransformMatrix`/`ModelMatrix`'s own composition — no shear modeling across non-uniform-scale levels), and overwrites the child's own `Transform2D`/`Transform3D` with the result. Run it after gameplay/physics updates a parent's transform and before any render system reads transforms, same convention as `CameraFollowSystem`. Destroying a parent orphans its children to world-space (last computed transform kept, `Parent` component removed) rather than destroying them — walk the graph yourself for cascading destroy. `Parent` is registered with `RegisterEngineComponents()` (type id `"Parent"`, JSON field `parentTag` resolved via the entity's tag at apply time); `LocalTransform2D`/`LocalTransform3D` round-trip the same way `Transform2D`/`Transform3D` do. See `docs/hierarchy.md`.
...
- **`Shader`** — wraps vertex+fragment GLSL source and compiles/links it into a GL program. GLSL source itself lives in individual files under `Rendering/Shaders/*.vert` / `*.frag` / `*.glsl` (one per shader stage, plus the odd shared snippet like `IblImportanceSampling.glsl` that's `.Replace()`-injected rather than compiled standalone) — not loaded from disk at runtime, but compiled in as `EmbeddedResource`s (`Yaeger.csproj`, logical name `Shaders.<file>`) and read via `EmbeddedShaderSource.Load("<file>")`. This keeps shaders in their own syntax-highlighted files for editing while requiring no runtime file I/O, same constraint the old inline-string-literal approach satisfied.

### 3D rendering (`Rendering/Renderer3D`, `Systems/MeshRenderSystem`)

- **`Renderer3D`** — forward renderer for 3D meshes with depth testing and back-face culling, independent of the 2D `Renderer`. One directional light + up to 16 `PointLight`s + 8 `SpotLight`s. Shading is Blinn-Phong by default or PBR metallic/roughness when `Material3D.UsePbr` is set (see `docs/lighting.md`, `docs/pbr.md`). Supports directional shadow mapping (`SetShadowMap`) and GPU skinning via a 128-bone matrix UBO (`SetBoneMatrices`). `SetEnvironmentMap`/`DisableIBL` swap the PBR path's flat ambient term for image-based lighting (irradiance + prefiltered specular + BRDF LUT) sampled from a prefiltered skybox; the Blinn-Phong path is unaffected either way (see `docs/pbr.md#image-based-lighting`). `DrawInstanced` draws N copies of a mesh sharing one material in a single `glDrawElementsInstanced` call, gated the same way as skinning via a `uInstanced` uniform in `Renderer3D.vert` (see `docs/instancing.md`); `DrawCallCount` (reset each `BeginFrame3D`) counts real GL draw calls issued, so an instanced group of any size counts as one.
- **`MeshRenderSystem`** — queries `MeshHandle` + `Transform3D` + `Material3D` entities and issues draws through `Renderer3D`; renders through the first `Camera3D` entity, frustum-culls entities carrying `Aabb3D`, collects light components, and routes entities with a `BonePalette` through the skinning path. Optional constructor args wire in a `SkyboxRenderer`/`CubemapRegistry` (renders any `Skybox` entity), a `ShadowMapRenderer` (depth pre-pass; configured by `ShadowSettings`, see `docs/shadows.md`), and an `EnvironmentMapRegistry` (auto-calls `SetEnvironmentMap`/`DisableIBL` based on whether the scene's `Skybox` has a prefiltered `EnvironmentMap` registered). Non-skinned entities surviving frustum culling are grouped by `(MeshHandle, Material3D)` via `MeshInstanceBatcher` (pure CPU-side, unit-tested); a group at or above `InstancingThreshold` (public, mutable, default `4`) draws through `Renderer3D.DrawInstanced` — one `glDrawElementsInstanced` call regardless of group size — instead of one `Renderer3D.Draw` call per entity. The shadow pre-pass groups the same way but by mesh alone (depth doesn't depend on material). Skinned entities always take the immediate per-entity path in both passes (a `BonePalette` is per-entity state). See `docs/instancing.md` and `Samples/MeshInstancingDemo`.
- **Registries** map CPU data to GPU resources and hand back value-type ECS handle components: `GpuMeshRegistry` (`MeshData` → `MeshHandle`), `CubemapRegistry` (6 face paths → `Skybox`), `TextureManager` (path-keyed texture cache), `SkeletonRegistry` (skeleton + clips → `SkeletonHandle`), `EnvironmentMapRegistry` (`Skybox` → prefiltered `EnvironmentMap`, produced by `IblPrefilter`'s offscreen convolution passes — irradiance cubemap, mip-chained specular prefilter, shared split-sum BRDF LUT — following `ShadowMapRenderer`'s pattern of a renderer owning its own FBO state; re-prefilters only when `Register` is called, not automatically).

### Post-processing (`Rendering/PostProcessStack.cs`, `Rendering/RenderTarget.cs`)

`RenderTarget` wraps an FBO with a colour texture (+ optional depth texture), resize-aware, following `ShadowMapRenderer`'s pattern of a renderer owning its own GL handles. `PostProcessStack` owns three of them — a scene target the game renders into, plus two ping-pong targets — and runs a chain of `IPostProcessEffect`s (`Effects`, a plain mutable list) over the rendered scene: each effect reads the previous pass's output and, for all but the last enabled effect, writes into whichever ping-pong target isn't the one it just read; the last enabled effect writes to the backbuffer (framebuffer `0`) directly, and zero enabled effects blits the scene target through unchanged. Which surface each pass reads/writes is decided by `PostProcessPlanner.Plan` — pure C#, no GL calls, unit-tested the same way `MeshInstanceBatcher` is. `PostProcessStack.Enabled = false` (or an empty/all-disabled `Effects` list) makes `Render(renderScene)` call `renderScene` straight against whatever's already bound, identical to not having a stack at all. `UnifiedRenderSystem`/`MeshRenderSystem` need no changes: both simply render into whichever framebuffer is currently bound, which `PostProcessStack.Render` arranges before calling them. Call `PostProcessStack.Resize` from `Window.OnResize` to keep the offscreen targets (and every effect's own targets) matched to the window size; render UI/inspector overlays *after* `PostProcessStack.Render` returns so they land on the backbuffer unaffected. Ships two built-in effects: `VignetteEffect` (single-pass edge-darkening + saturation/tint grade) and `BloomEffect` (multi-pass: brightness threshold → separable Gaussian blur over `BlurIterations` pairs at half resolution → additive composite) — both LDR only, no tone mapping. See `docs/post-processing.md` and `Samples/PostProcessingDemo`.
...
Value-type ECS components — 2D: `Sprite`, `Transform2D`, `Camera2D`, `Color`, `SpriteSheet`, `Animation`, `AnimationState`, `AnimationStateMachine`, `Text`, `RenderLayer`, `ParallaxLayer`, `Tilemap` (+ `Tileset`); 3D: `Transform3D`, `Camera3D`, `Material3D`, `MeshHandle`, `Aabb3D`, `DirectionalLight`, `PointLight`, `SpotLight`, `Skybox`, `SkeletonHandle`, `AnimationPlayer`, `BonePalette`.

`SpriteSheet` calculates normalised UV rectangles per frame index via `GetFrameUv(int frameIndex)`.

`Sprite.FlipX`/`FlipY` mirror the rendered image horizontally/vertically without touching `Transform2D` (decoupling facing from scale, so it doesn't leak into colliders). `UnifiedRenderSystem` implements this by swapping the submitted quad's UV min/max via `Sprite.ApplyFlip(uvMin, uvMax, flipX, flipY)` — this keeps flipped sprites on the same batched draw path (no extra draw calls) and works identically for a plain `Sprite`'s full-texture UV or a `SpriteSheet` frame's sub-rectangle. A `SpriteSheet`+`AnimationState`-driven entity can carry an extra co-located `Sprite` purely for its `FlipX`/`FlipY` — `UnifiedRenderSystem` reads that component's flip flags only, its `TexturePath` is unused since the `SpriteSheet`'s texture is authoritative. `AnimationSystem` preserves `FlipX`/`FlipY`/`Tint` on the `Sprite` it writes back each frame advance for plain-`Sprite` animations. Serializes as optional `flipX`/`flipY` JSON fields on `Sprite` (omitted when `false`).

`AnimationStateMachine` (+ `AnimationStateMachineSystem`) is a small helper for switching between named `Animation` clips (idle/run/jump/fall) without game code manually swapping `Animation`/`AnimationState` components — deliberately minimal, no blend trees or transition-condition DSL. Call `AnimationStateMachineSystem.Play(entity, stateName)` to request a switch (throws if the entity lacks the component or the name isn't defined); `Update(deltaTime)` applies the pending request and bootstraps the initial clip on an entity that hasn't rendered a frame yet. Switching to a different state always restarts at frame 0; replaying the already-active state only restarts it if `RestartOnReplay` is set at construction (default `false`, so a looping state doesn't stutter every time game code reconfirms it). Completion of a non-looping state is reported the normal way — `AnimationState.IsFinished`. Run `AnimationStateMachineSystem.Update` *before* `AnimationSystem` each frame so a same-frame switch takes effect before frame-timing advances. See `docs/animation-system.md`.
...
**Camera follow** (`Systems/CameraFollowSystem.cs`, `Yaeger` assembly only — it takes an optional `Window` for aspect ratio): `CameraFollow` (target entity, exponential `Smoothing` rate, `DeadzoneHalfExtents`, velocity-based `LookAheadTime`) attached alongside a `Camera2D` makes `CameraFollowSystem.Update(dt)` track the target's `Transform2D` every step — framerate-independent smoothing (`Smoothing <= 0` snaps instantly), a deadzone box the target can move within before the camera reacts, and look-ahead that biases the view towards a `Velocity2D`-carrying target's direction of travel (no-op without one). Pair with `CameraBounds` (a world-space rect, optionally derived via `CameraBounds.FromTilemap`) on the same entity to additionally clamp the camera's visible span — half-extents `(aspectRatio / Zoom, 1 / Zoom)`, per `Camera2D`'s remarks — so it never shows past a level's edges; an axis where the bounds are narrower than the viewport is centered on the bounds' own midpoint instead of clamped. If the target entity is destroyed or otherwise loses its `Transform2D`, the camera holds its last position rather than snapping or throwing. Run this system after gameplay/physics so it reads final positions. See `Samples/CameraDemo` (space toggles follow mode).
