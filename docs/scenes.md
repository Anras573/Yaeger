...
All engine-provided serializers support the write direction. `RegisterEngineComponents()` registers serializers for the 2D components (`Sprite`, `Transform2D`, `SpriteSheet`, `Animation`, `AnimationState`, `AnimationStateMachine`, `RenderLayer`, `Tilemap`, `Camera2D`, `Text`, `ParticleEmitter`, `ParallaxLayer`, `BoxCollider2D`, `CircleCollider2D`, `RigidBody2D`, `Velocity2D`, `PhysicsMaterial`) and the 3D components (`Transform3D`, `Camera3D`, `Material3D`, `DirectionalLight`, `PointLight`, `SpotLight`). `MeshHandle` is deliberately excluded — its `Id` is a runtime-assigned `GpuMeshRegistry` key that isn't portable across runs, so meshes are re-assigned in code rather than persisted. `Text` is the one exception worth calling out: it round-trips its `FontHandle` (a font *path*, resolved the same way `Sprite.TexturePath` is) rather than the native `Yaeger.Font.Font` instance it may have been constructed with — the same handle-vs-native-instance split `MeshHandle` avoids by not being persisted at all, except a font path *is* portable across runs. The screen-space UI components (`UiRect`, `UiPanel`, `UiButton`, `UiLabel`) are not yet registered — whether screen-space UI belongs in world scenes is still an open design question. Custom serializers opt in by overriding the default `TrySerialize` method on `IComponentSerializer`; they must return a `JsonObject` that includes a non-empty `"type"` field — `SceneSaver` throws `SceneSaveException` if that contract is violated.
