...
| 3D camera rig | `FreeFlyCameraSystem` in the [Cornell Box](../Samples/FeatureGallery/Scenes/CornellBox)/Sponza/Skinned Mesh scenes; `Transform3D` bridge, `LookAtTarget`, `CameraShake` in the [Camera Rig](../Samples/FeatureGallery/Scenes/CameraRig) gallery scene | `--project Samples/FeatureGallery/FeatureGallery.csproj -- --scene "Camera Rig"` |
