...
/// <para>
/// Lives under <c>ECS/Serializers/Native/</c> rather than alongside the other serializers: it
/// references <see cref="Text"/>, which holds a native <c>Yaeger.Font.Font</c> field with no
/// <c>Yaeger.Core</c> equivalent, so it can only compile into the native <c>Yaeger</c> assembly —
/// see <c>Yaeger.Core.csproj</c>/<c>Yaeger.csproj</c> for how that folder is routed.
/// </para>
/// </remarks>
