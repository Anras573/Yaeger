...
namespace FeatureGallery.Scenes.CornellBox;

/// <summary>
/// Cornell Box demo — classic CG test scene built entirely from procedural geometry. No external
/// assets required. Demonstrates directional-light shadow mapping, coloured point lights,
/// additive/transparent material blend modes, 3D particle emitters, and the live editor overlay.
/// </summary>
public sealed class CornellBoxScene : IDemoScene
{
    private World? _world;
    private GpuMeshRegistry? _registry;
    private TextureManager? _textures;
    private ParticleSystem3D? _particleSystem3D;
    private Renderer3D? _renderer3D;
    private ShadowMapRenderer? _shadowMapRenderer;
    private MeshRenderSystem? _meshRenderSystem;
    private ParticleRenderSystem3D? _particleRenderSystem3D;
    private FreeFlyCameraSystem? _freeFlySystem;
    private ImGuiInspector? _inspector;

    public string Name => "Cornell Box";
    public string Description => "Procedural PBR test scene with shadows, lights, and particles";
    ...
    public void Load(Window window)
    {
        ...
        )
        {
            ...
            world.AddComponent(
                ...
                new Transform3D(
                    position,
                    Quaternion.CreateFromAxisAngle(Vector3.UnitY, rotationY),
                    scale
                )
            );
            world.AddComponent(entity, material);
        ...
        // Additive: adds its colour to whatever is behind it (glBlendFunc(SrcAlpha, One)), so it
        // can only brighten the frame — never darken it. Contrast this with Glass below over the
        // same background.
        ...
        // Transparent: interpolates between its colour and whatever is behind it
        // (glBlendFunc(SrcAlpha, OneMinusSrcAlpha)), so it tints/darkens rather than brightens.
        ...
        // Additive vs. transparent demo — two floating panels facing the camera, side by side over
        // the same back-wall background, so the difference between blend modes is visible at a
        // glance: the additive panel brightens the wall behind it, the transparent panel
        // tints/darkens it.
        AddSurface(
            "glow_panel",
            MeshFactory.CreateQuad(
        ...
        // Coloured point lights — demonstrate multiple light sources casting distinct pools of
        // colour across the walls and boxes. Each is placed via a Transform3D; MeshRenderSystem
        // queries them.
        ...
        const string particleTexture = "Assets/CornellBox/particle.png";

        // Embers — additive, round billboards drifting up from the short box. VelocityStretch
        // stays 0 (the default) so these stay round puffs of glow rather than streaks.
        ...
        // Sparks — additive too, but VelocityStretch elongates each billboard into a streak
        // aligned with its direction of travel, contrasting with the round embers above.
        ...
        // Directional-light shadow mapping. The orthographic frustum is sized to frame the
        // 2×2×2 room (centred on the camera target), and PCF softens the cast-shadow edges.
        ...
        _particleRenderSystem3D = new ParticleRenderSystem3D(
            renderer3D,
            _textures,
            world,
            window,
        ...
        _freeFlySystem = new FreeFlyCameraSystem(world, cameraEntity, moveSpeed: 3f);
    ...
    public void Update(float deltaTime)
    {
        ...
        _particleSystem3D?.Update(deltaTime);
    }
    ...
    public void Render(float deltaTime)
    {
        ...
        // particles are alive in it — orbit the camera (right-mouse-drag) to see the billboards
        // stay camera-facing from any angle.
        _particleRenderSystem3D?.Render();
